...
(() => {
    ...
    // Kontrolowana ramka wykonuje wyłącznie natywne, pośrednie formularze
    // logowania. Nakładka nie może montować się ponownie wewnątrz tej ramki.
    if (window.name === "trans-assistant-intranet-login-flow") return;
    ...
    function readPopupPeriod(name, fallback) {
        try {
            const value = window.opener?.document?.querySelector(`select[name="${name}"]`)?.value;
    ...
    async function loadCorrectionRecord(orderNumber) {
        ...
        const initial = await fetchHtmlDocument(correctionUrl);
        const searchForm = Array.from(initial.document.forms || []).find(form =>
        ...
        searchData.set("numer_zlecenia", orderNumber);
        searchData.set("rok", readPopupPeriod("rok", today.getFullYear()));
        searchData.set(
            "miesiac",
            readPopupPeriod("miesiac", String(today.getMonth() + 1).padStart(2, "0"))
        );
        const searchAction = new URL(
            searchForm.getAttribute("action") || initial.url,
            initial.url
        ).href;
        const result = await fetchHtmlDocument(searchAction, {
            method: String(searchForm.method || "POST").toUpperCase(),
            body: searchData
        });
        return {
            ...result,
            form: findUniqueCorrectionForm(result.document)
        };
    }
    ...
    function submitCorrectionFormNatively(record, overrides) {
        return new Promise((resolve, reject) => {
            ...
            form.enctype = record.form.enctype || "multipart/form-data";
            form.acceptCharset = "iso-8859-2";
            form.target = frameName;

            const values = new Map();
            for (const control of Array.from(record.form.elements || [])) {
                if (!control.name || control.disabled || /^(?:submit|button|image|file|reset)$/i.test(control.type)) continue;
                if ((control.type === "checkbox" || control.type === "radio") && !control.checked) continue;
                values.set(control.name, control.value);
            }
            Object.entries(overrides).forEach(([name, value]) => values.set(name, String(value ?? "")));
            ...
            frame.addEventListener("load", () => {
                if (!submitted) {
                    submitted = true;
                    logOrderSave("correction-native", form, submitter);
                    submitter.click();
                    return;
                }
    ...
    async function saveCorrectionFields(orderNumber, values, preparedRecord = null) {
        ...
        let saved = await submitCorrectionFormNatively(record, {
            n_miejsce: values.deliveryPlace,
            wartosc_termin_zaplaty: values.paymentTerm,
            n_odleglosc: values.distance
        });
        if (
            saved.savedDelivery !== values.deliveryPlace
            || saved.savedPayment !== values.paymentTerm
            || saved.savedDistance !== values.distance
        ) {
    ...
    function resolveDateCorrectionUrl(correctionRecord) {
        const links = Array.from(correctionRecord?.document?.querySelectorAll("a[href]") || []);
        for (const link of links) {
            const href = String(link.getAttribute("href") || "");
            if (!/zmien_date\.php/i.test(href)) continue;
            const popupMatch = /["']([^"']*zmien_date\.php(?:\?[^"']*)?)["']/i.exec(href);
            const target = popupMatch?.[1] || href;
            if (/^javascript:/i.test(target)) continue;
            return new URL(target, correctionRecord.url).href;
        }
        throw new Error("Nie znaleziono oryginalnego odnośnika do zmiany dat.");
    }

    async function loadDateCorrectionRecord(correctionRecord) {
        const dateUrl = resolveDateCorrectionUrl(correctionRecord);
        const result = await fetchHtmlDocument(dateUrl);
        return {
            ...result,
            form: findUniqueDateCorrectionForm(result.document)
        };
    }
    ...
    function submitDateCorrectionFormNatively(record, loadingDate, deliveryDate) {
        return new Promise((resolve, reject) => {
            ...
            const frame = document.createElement("iframe");
            const form = document.createElement("form");
            const frameName = `ta-date-correction-${token}`;
            const nativeSubmit = record.form.querySelector('input[type="image"]');
            if (!nativeSubmit) {
            ...
            values.set("data_zaladunku", loadingDate);
            values.set("data_dostawy", deliveryDate);
            values.forEach((value, name) => {
                const input = document.createElement("input");
                input.type = "hidden";
                input.name = name;
            ...
            const submitter = nativeSubmit.cloneNode(false);
            form.appendChild(submitter);
            ...
            frame.addEventListener("load", () => {
                if (!submitted) {
                    submitted = true;
                    logOrderSave("date-correction-native", form, submitter);
                    submitter.click();
                    return;
                }
                try {
    ...
    function createQuickCorrectionPanel(orderId) {
        ...
        panel._taUseNativeValues = ({ paymentTerm = "", deliveryPlace = "" } = {}) => {
            paymentInput.value = String(paymentTerm || "").trim() || "55";
        ...
        panel._taSave = async () => {
            ...
            const paymentTerm = String(paymentInput.value || "").trim() || "55";
            ...
            if (!/^\d{1,3}$/.test(paymentTerm)) {
                setStatus("Termin płatności musi być liczbą od 0 do 999 dni.", "error");
                paymentInput.focus();
                return false;
            }
            if (!deliveryPlace) {
            ...
            if (!/^\d+(?:[.,]\d+)?$/.test(distance)) {
                setStatus("Odległość musi być prawidłową liczbą.", "error");
            ...
            try {
                ...
                console.log("[CEMET SAVE]", {
                    kind: "correction",
                    form: "administracja/korekta_danych.php",
                    action: "/spedycja_uss_2022/administracja/korekta_danych.php",
                    method: "POST",
                    submitter: "embedded-correction"
                });
                const saved = await saveCorrectionFields(
                    orderNumber,
                    { paymentTerm, deliveryPlace, distance },
                    correctionRecord
                );
                if (orderDetailsPageState?.paymentField) {
                    orderDetailsPageState.paymentField.value = saved.savedPayment;
                }
                if (panel._taDistanceInput) panel._taDistanceInput.value = saved.savedDistance;
                orderDetailsPageState?.embeddedCorrection?.updateOriginalValues?.(saved);
                if (panel._taDateCorrection?.isDirty?.()) {
                    const datesSaved = await panel._taDateCorrection.save(correctionRecord);
                    if (!datesSaved) throw new Error("Nie udało się zapisać zmienionych dat.");
                }
                setStatus("Zapisano i potwierdzono w korekcie danych.", "success");
                return true;
            } catch (error) {
    ...
    function embedCorrectionControls(panel, detailsRows) {
        ...
        const nativePaymentText = String(
            nativePaymentControl?.value || paymentCell.textContent || ""
        ).trim();
        const nativePaymentValue = /^\d{1,3}$/.test(nativePaymentText) ? nativePaymentText : "55";
        ...
        paymentSave.addEventListener("click", () => panel._taSave());
        paymentEditor.appendChild(paymentSave);
        paymentCell.append(paymentOriginal, paymentEditor);

        return {
            updateOriginalValues(saved) {
                deliveryOriginal.textContent = saved.savedDelivery;
                const nativePayment = paymentOriginal.querySelector('input, select, textarea');
                if (nativePayment) nativePayment.value = saved.savedPayment;
            }
        };
    }
    ...
    function embedDateCorrectionControls(panel, detailsRows) {
        ...
        const prepareRecord = correctionRecord => {
            if (record) return Promise.resolve(record);
            if (preparePromise) return preparePromise;
            preparePromise = (async () => {
                const baseRecord = correctionRecord || await panel._taLoadCorrectionRecord?.();
                if (!baseRecord) throw new Error("Nie znaleziono danych korekty zlecenia.");
                record = await loadDateCorrectionRecord(baseRecord);
            ...
            async save(correctionRecord = null) {
                ...
                savePromise = (async () => {
                    ...
                    if (
                        ...
                        const refreshed = await fetchHtmlDocument(record.url);
                        const refreshedForm = findUniqueDateCorrectionForm(refreshed.document);
                        record = { ...refreshed, form: refreshedForm };
                        saved = {
                            ...saved,
                    ...
                    if (kind === "main") {
                        ...
                        if (saved === false) {
                            console.error("[CEMET SAVE ERROR]", {
                                kind: "correction-before-main",
                                message: "Korekta terminu lub miejsca dostawy nie została zapisana; główny formularz zostanie wysłany normalnie."
                            });
                        }
                    }
